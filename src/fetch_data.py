...
@click.option('--days_back', default=7, type=click.IntRange(min=1),
              help='Number of days back to fetch messages. Must be a positive integer.')
@click.option('--file_name', default='chat_backup.txt', help='File name for saving data. Default is chat_backup.txt')
        ...
        with open(file_name, 'w') as file:
            async for message in client.iter_messages(chat):
                ...
                if start_date > message_date:
                    break
                file.write(f"{message_date:%Y-%m-%d %H:%M:%S} {message.sender.first_name} {message.sender.last_name}: "
                           f"{message.text}\n")
